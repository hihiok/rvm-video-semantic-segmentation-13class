#!/usr/bin/env python3
"""Train the existing FSD/UltraFace scene network as an 8-label tagger.
...
YUV444 scene transforms, and changes only the training objective/data interface
from single-label CE to partial-label BCEWithLogitsLoss.
"""
...
def parse_args():
    p = argparse.ArgumentParser(description="FSD 8-label multi-label scene training")
    p.add_argument("--fsd-root", type=Path, required=True)
    p.add_argument("--data-root", type=Path, required=True)
    p.add_argument("--output-dir", type=Path, required=True)
    p.add_argument("--input-size", type=int, default=240)
...
def per_class_metrics(gt, scores, thresholds):
    ...
    for j, label in enumerate(LABELS):
        ...
        rows.append({
            "label": label, "display_name": DISPLAY_NAMES[label], "threshold": float(thresholds[j]),
            "known": int(mask.sum()), "positive": int((y == 1).sum()), "negative": int((y == 0).sum()),
            "precision": float(precision), "recall": float(recall), "f1": float(f1),
            "accuracy": float(accuracy), "balanced_accuracy": float(balanced), "ap": binary_ap(y, s),
        })
    summary = {
...
def calibrate_thresholds(gt, scores):
    ...
    for j in range(len(LABELS)):
        mask = gt[:, j] >= 0
        y = gt[mask, j].astype(np.int64); s = scores[mask, j]
...
def evaluate(model, loader, device, use_amp):
    model.eval(); all_gt = []; all_scores = []
    with torch.no_grad():
        for x, y in loader:
            x = x.to(device, non_blocking=True); y = y.numpy()
            ...
            all_scores.append(torch.sigmoid(logits).float().cpu().numpy()); all_gt.append(y)
...
def main():
    args = parse_args()
    os.environ.setdefault("OMP_NUM_THREADS", str(args.cpu_threads)); os.environ.setdefault("MKL_NUM_THREADS", str(args.cpu_threads))
    torch.set_num_threads(args.cpu_threads); set_seed(args.seed)
    ...
    train_tf = YUV444TrainAugmentation_scene(fd_config.image_size)
    eval_tf = YUV444TestTransform_scene(fd_config.image_size)
    train_ds = ManifestDataset(train_records, train_tf); val_ds = ManifestDataset(val_records, eval_tf); test_ds = ManifestDataset(test_records, eval_tf)
    ...
    pos_weight, pos_count, neg_count = compute_pos_weight(train_records); pos_weight = pos_weight.to(device)
    logging.info("TRAIN_SUPERVISION %s", {l: {"pos": int(pos_count[i]), "neg": int(neg_count[i]), "pos_weight": float(pos_weight[i].cpu())} for i,l in enumerate(LABELS)})
    ...
    if args.resume_train_state:
        ...
        scaler.load_state_dict(ck.get("scaler", scaler.state_dict())); start_epoch = int(ck["epoch"]) + 1; best_f1 = float(ck.get("best_macro_f1", -1.0))

    history = args.output_dir / "metrics.jsonl"
    for epoch in range(start_epoch, args.epochs):
        model.train(); loss_sum = 0.0; steps = 0; t0 = time.time()
        for step, (x, y) in enumerate(train_loader):
            ...
            if args.grad_clip > 0:
                scaler.unscale_(optimizer); nn.utils.clip_grad_norm_(model.parameters(), args.grad_clip)
            scaler.step(optimizer); scaler.update()
            loss_sum += float(loss.item()); steps += 1
            if step > 0 and step % args.print_every == 0:
                logging.info("epoch=%03d step=%05d/%05d loss=%.5f lr=%.3e", epoch, step, len(train_loader), loss_sum/max(steps,1), optimizer.param_groups[0]["lr"])
        scheduler.step()
        gt_val, sc_val = evaluate(model, val_loader, device, args.amp)
        rows, summary = per_class_metrics(gt_val, sc_val, np.full(len(LABELS), 0.5))
        rec = {"epoch": epoch, "train_loss": loss_sum/max(steps,1), "seconds": time.time()-t0, **summary}
        with history.open("a", encoding="utf-8") as f: f.write(json.dumps(rec) + "\n")
        logging.info("VAL %s", json.dumps(rec))
        state = {"epoch": epoch, "model": model.state_dict(), "optimizer": optimizer.state_dict(), "scheduler": scheduler.state_dict(), "scaler": scaler.state_dict(), "best_macro_f1": best_f1, "labels": LABELS, "input_size": args.input_size}
        torch.save(state, args.output_dir / "last_train_state.pth")
        if summary["macro_f1"] > best_f1:
            best_f1 = summary["macro_f1"]; state["best_macro_f1"] = best_f1
            torch.save(state, args.output_dir / "best_train_state.pth"); save_csv(args.output_dir / "best_val_per_class_0p5.csv", rows)

    best = torch.load(args.output_dir / "best_train_state.pth", map_location="cpu")
    model.load_state_dict(best["model"], strict=True)
    gt_val, sc_val = evaluate(model, val_loader, device, args.amp); thresholds = calibrate_thresholds(gt_val, sc_val)
    gt_test, sc_test = evaluate(model, test_loader, device, args.amp); test_rows, test_summary = per_class_metrics(gt_test, sc_test, thresholds)
    save_csv(args.output_dir / "test_per_class_calibrated.csv", test_rows)
    (args.output_dir / "test_summary.json").write_text(json.dumps(test_summary, indent=2, ensure_ascii=False), encoding="utf-8")
    threshold_dict = {l: float(thresholds[i]) for i,l in enumerate(LABELS)}
    (args.output_dir / "thresholds.json").write_text(json.dumps(threshold_dict, indent=2, ensure_ascii=False), encoding="utf-8")
    # FSD-compatible classifier-only checkpoint, loadable by existing net.load().
    unwrap(model).save(str(args.output_dir / "best_fsd_multilabel8.pth"))
    metadata = {
        "labels": LABELS, "display_names": DISPLAY_NAMES, "thresholds": threshold_dict,
        "input_size_argument": args.input_size, "fd_config_image_size": list(fd_config.image_size),
        "factory": "create_Mb_Tiny_RFB_fd_3_scene_noRFB(2, 8)",
        "loss": "masked BCEWithLogitsLoss", "output": "8 raw logits; sigmoid + per-class threshold",
    }
    (args.output_dir / "deployment_metadata.json").write_text(json.dumps(metadata, indent=2, ensure_ascii=False), encoding="utf-8")
