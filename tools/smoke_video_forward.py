...
def main():
    ...
    images = torch.rand(1, args.time, 3, args.size, args.size, device=args.device)
    targets = torch.randint(0, 13, (1, args.time, args.size, args.size), device=args.device)
    logits, *states = model(images)
    assert logits.shape == (1, args.time, 13, args.size, args.size), logits.shape
