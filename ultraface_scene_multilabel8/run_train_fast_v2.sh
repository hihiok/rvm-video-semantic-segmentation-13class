...
GPU="${GPU:-0}"; BATCH="${BATCH:-128}"; WORKERS="${WORKERS:-16}"; PREFETCH="${PREFETCH:-2}"; RESUME="${RESUME:-}"
export CUDA_VISIBLE_DEVICES="${GPU}" OMP_NUM_THREADS=4 MKL_NUM_THREADS=4 OPENBLAS_NUM_THREADS=4 NUMEXPR_NUM_THREADS=4
mkdir -p "${OUT}"
ARGS=(--data-root "${DATA_ROOT}" --output-dir "${OUT}" --epochs 30 --batch-size "${BATCH}" --workers "${WORKERS}" --prefetch-factor "${PREFETCH}" --lr 1e-2 --momentum 0.9 --weight-decay 1e-4 --milestones 20,27 --gamma 0.1 --cpu-threads 4 --amp --log-every 50)
